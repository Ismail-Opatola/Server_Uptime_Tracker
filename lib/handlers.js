...
const debug = util.debuglog("performance"); // NODE_DEBUG=performance node index.js
          ...
          if (!err && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              const parseUrl = _url.parse(`protocol://${url}`, true),
                hostName =
                  typeof parseUrl.hostname == "string" &&
                  parseUrl.hostname.length > 0
                    ? parseUrl.hostname
                    : false;
              ...
              dns.resolve(hostName, (err, records) => {
                if (!err && records) {
                  ...
                  // create the check obj an include the user's phone
                  const checkObject = {
                    id: checkId,
                    userPhone: userPhone,
                    protocol: protocol,
                  ...
                  callback(400, {
                    Error: `The hostname of the URL entered did not resolve into any DNS entries`
                  });
                }
              });
            } else {
